...
bool MEMSSensor::processVibrationData(const MEMSData& raw_data, VibrationAnalysis& analysis) {
    ...
    // Calculate displacement from RMS velocity and frequency
    if (valid_x && analysis.vibration_freq_x > 0.0f) {
        analysis.displacement_x_um = (analysis.rms_velocity_x / (2.0f * PI * analysis.vibration_freq_x)) * 1000.0f;
    } else {
        analysis.displacement_x_um = 0.0f;
    }
    if (valid_y && analysis.vibration_freq_y > 0.0f) {
        analysis.displacement_y_um = (analysis.rms_velocity_y / (2.0f * PI * analysis.vibration_freq_y)) * 1000.0f;
    } else {
        analysis.displacement_y_um = 0.0f;
    }
    if (valid_z && analysis.vibration_freq_z > 0.0f) {
        analysis.displacement_z_um = (analysis.rms_velocity_z / (2.0f * PI * analysis.vibration_freq_z)) * 1000.0f;
    } else {
        analysis.displacement_z_um = 0.0f;
    }
